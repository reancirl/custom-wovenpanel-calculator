const resolveCartSectionsUrl = () => {
  try {
    if (window.routes && window.routes.cart_url) {
      return new URL(window.routes.cart_url, window.location.origin).pathname;
    }
  } catch (error) {
    console.warn('Unable to resolve cart sections URL', error);
  }
...
class CartRemoveButton extends HTMLElement {
  constructor() {
    ...
    this.addEventListener('click', (event) => {
      ...
      const removeLink = event.target.closest('a[href]') || this.querySelector('a[href]');
      let line = this.dataset.index;

      if (removeLink) {
        try {
          const removeUrl = new URL(removeLink.getAttribute('href'), window.location.origin);
          line = removeUrl.searchParams.get('line') || line;
        } catch (error) {
          // Fall back to the rendered line index when URL parsing fails.
        }
      }
...
class CartItems extends HTMLElement {
  constructor() {
    ...
    this.sectionsUrl = resolveCartSectionsUrl();
